...
impl Database {
    ...
    /// Delete snapshot embeddings when their snapshot is deleted (cascade via chunk_id).
    /// Called explicitly because vec0 tables don't honour ON DELETE CASCADE.
    ...
    /// Return all versions of a symbol across all snapshots, ordered by snapshot creation time.
    /// Also includes the live index version (snapshot_sha = None).
    /// `name_fragment` is matched as a suffix (`% || fragment`).
    pub fn symbol_history(&self, name_fragment: &str) -> Result<Vec<SymbolVersion>> {
        let pattern = format!("%{name_fragment}");

        // Live index version(s).
        let mut stmt = self.conn.prepare(
        ...
        // Snapshot versions — all snapshots, ordered by created_at ASC.
        ...
        // Combine: snapshots first (ASC), then live.
        let mut all = snaps;
        all.extend(live);
        Ok(all)
    }
